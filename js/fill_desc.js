import { OpenModal, ResolvedTaskList, Storage } from "./data.js";
...
function fill_desc() {
    for (let index = 0; index < Storage.length; index++) {
        ...
        try {
            JSON.parse(Storage.getItem(Key)).status !== 'Решена' ? OpenModal.after(create_task_element(Key)) : ResolvedTaskList.appendChild(create_task_element(Key)); 
        } catch (error) {
