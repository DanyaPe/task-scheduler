...
function fill_desc() {
    for (let index = 0; index < Storage.length; index++) {
        ...
        try {
            JSON.parse(Storage.getItem(Key))['Статус задачи'] !== 'Решена' ? NewTaskList.appendChild(create_task_element(Key)) : ResolvedTaskList.appendChild(create_task_element(Key)); 
        } catch (error) {
